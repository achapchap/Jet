...
subroutine spline(xs,zs,xs2,zs2,tp,yp1,ypn,n,nmax)
 ...
 real*8, dimension(nmax)    :: xs,zs,xs2,zs2,tp
...
! interpolation of zs
 ...
 if (yp1.gt.1.d+30) then
   ...
   zs2(1) = -0.5d0
   u(1)  = (3.d0/(tp(2)-tp(1)))*((zs(2)-zs(1))/(tp(2)-tp(1)) - yp1)
 end if
 do i = 2,n-1
   sig     = (tp(i)-tp(i-1))/(tp(i+1)-tp(i-1))
   p       = sig*zs2(i-1)+2.d0
   zs2(i)   = (sig-1.d0)/p
   u(i)    = (6.d0*((zs(i+1)-zs(i))/(tp(i+1)-tp(i))-(zs(i)-zs(i-1)) &
          /(tp(i)-tp(i-1)))/(tp(i+1)-tp(i-1)) - sig*u(i-1))/p
 enddo
 if (ypn.gt.1.d+30) then
   qn = 0.d0
   ...
   qn = 0.5d0
   un = (3.d0/(tp(n) - tp(n-1)))*(ypn - (zs(n)-zs(n-1))/(tp(n)-tp(n-1)))
 end if
 zs2(n) = (un - qn*u(n-1))/(qn*zs2(n-1)+1.d0)
 do k = n-1,1,-1
   zs2(k) = zs2(k)*zs2(k+1)+u(k)
 enddo
